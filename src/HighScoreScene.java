...
class HighScoreScene extends SirtetScene implements KeyListener {
    private SirtetLabel[] indexLabels = new SirtetLabel[10];
    private SirtetLabel[] nameLabels = new SirtetLabel[10];
    private SirtetLabel[] scoreLabels = new SirtetLabel[10];

    public HighScoreScene() {
        super(true);
        add(new LabelCenter("Highscores", 60, 50));
        add(new LabelCenter("ESC to Return", 30, 725));
        loadLabels();
    ...
    public void loadLabels() {
        for (int i = 0; i < 10; i++) {
            indexLabels[i] = new LabelRight(i + 1 + ": ", 30, 110, 160 + i * 50);
            nameLabels[i] = new LabelRight("", 30, 0, 0);
            scoreLabels[i] = new LabelRight("", 30, 0, 0);
        }
    }
